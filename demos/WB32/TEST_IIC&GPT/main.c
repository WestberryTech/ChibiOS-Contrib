...
I2CConfig i2cconfig = { OPMODE_I2C , 350000};
...
SPIConfig spiconfig = { NULL,NULL,NULL,
                        ...
                        SPI_CR0_FRF_SPI,
                        SPI_NSS_0 };
GPTConfig gptconfig = {10000, gpt3cb };
/*===========================================================================*/
/* Generic code.                                                             */
/*===========================================================================*/
...
void EXTI0_Config(void)
{
  ...
  // _pal_lld_setgroupmode(GPIOA, GPIO_Pin_0, PAL_WB32_PUPDR_PULLDOWN | PAL_WB32_PUPDR_PULLDOWN);

  pal_lld_setgroupmode(GPIOA, 1, 0, PAL_WB32_PUPDR_PULLDOWN | PAL_WB32_PUPDR_PULLDOWN);
...
static void gpt3cb(GPTDriver *gptp) {
  palTogglePad(IOPORT2, 14);
}

uint8_t rx_buffer[256];
uint8_t tx_buffer[256]={0x06,0x07,0x08,0x09,0x0a,0x0b,0x0c,0x0d,0x0e,0x02};
...
int main(void)
{
  ...
  msg = i2cMasterTransmitTimeout(&I2CD2,0xA0 >> 1,tx_buffer,10,rx_buffer,0,WB32_I2C_BUSY_TIMEOUT);
  osalDbgCheck(MSG_OK == msg);
  chThdSleepMilliseconds(10);
  
  msg = i2cMasterTransmitTimeout(&I2CD2,0xA0 >> 1,tx_buffer,1,rx_buffer,5,WB32_I2C_BUSY_TIMEOUT);
  osalDbgCheck(MSG_OK == msg);
  chThdSleepMilliseconds(10);
  
  uart_len = strlen((const char*)rx_buffer);
  uartSendTimeout(&UARTD1 , (size_t *)&uart_len , rx_buffer , TIME_INFINITE);//
  chThdSleepMilliseconds(100);
  
  msg = i2cMasterTransmitTimeout(&I2CD2,0xA0 >> 1,tx_buffer,6,rx_buffer,4,WB32_I2C_BUSY_TIMEOUT);
  osalDbgCheck(MSG_OK == msg);
  chThdSleepMilliseconds(10);
  ...
  msg = i2cMasterReceiveTimeout(&I2CD2,0xA0 >> 1,rx_buffer,5,WB32_I2C_BUSY_TIMEOUT);
  osalDbgCheck(MSG_OK == msg);
