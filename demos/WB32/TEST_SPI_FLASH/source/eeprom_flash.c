...
static uint8_t eeprom_write_direct_entry(uint16_t Address) {
    ...
    if (wvalue == FEE_EMPTY_WORD) {
        ...
        wvalue = ~((uint16_t *)(&DataBuf[Address & 0xFFFE]))[0];
...
uint8_t EEPROM_WriteDataByte(uint16_t Address, uint8_t DataByte) {
    ...
    FLASH_Status status = (FLASH_Status)eeprom_write_direct_entry(Address);
    if (!status) {
        /* Otherwise append to the write log */
        if (Address < FEE_BYTE_RANGE) {
            status = (FLASH_Status)eeprom_write_log_byte_entry(Address);
        } else {
            status = (FLASH_Status)eeprom_write_log_word_entry(Address & 0xFFFE);
        }
    }
...
uint8_t EEPROM_WriteDataWord(uint16_t Address, uint16_t DataWord) {
    ...
    FLASH_Status final_status = FLASH_COMPLETE;
    if (Address % 2) {
        final_status        = (FLASH_Status)EEPROM_WriteDataByte(Address, DataWord);
        FLASH_Status status = (FLASH_Status)EEPROM_WriteDataByte(Address + 1, DataWord >> 8);
        if (status != FLASH_COMPLETE) final_status = status;
    ...
    *(uint16_t *)(&DataBuf[Address]) = DataWord;
    ...
    /* perform the write into flash memory */
    /* First, attempt to write directly into the compacted flash area */
    final_status = (FLASH_Status)eeprom_write_direct_entry(Address);
    if (!final_status) {
        /* Otherwise append to the write log */
        ...
        if (Address < FEE_BYTE_RANGE) {
            ...
            if ((uint8_t)oldValue != (uint8_t)DataWord) {
                final_status = (FLASH_Status)eeprom_write_log_byte_entry(Address);
            }
            FLASH_Status status = FLASH_COMPLETE;
            /* Only write a byte if it has changed */
            if ((oldValue >> 8) != (DataWord >> 8)) {
                status = (FLASH_Status)eeprom_write_log_byte_entry(Address + 1);
            }
            if (status != FLASH_COMPLETE) final_status = status;
        } else {
            final_status = (FLASH_Status)eeprom_write_log_word_entry(Address);
        }
    }
    if (final_status != 0 && final_status != FLASH_COMPLETE) {
        eeprom_printf("EEPROM_WriteDataWord [STATUS == %d]\n", final_status);
...
void eeprom_read_block(void *buf, const void *addr, size_t len) {
    ...
    if (len && (uintptr_t)src % 2) {
        ...
        *dest++ = EEPROM_ReadDataByte((uintptr_t)src++);
        --len;
    }
    ...
    while (len > 1) {
        value = EEPROM_ReadDataWord((uintptr_t)((uint16_t *)src));
        if (aligned) {
            *(uint16_t *)dest = value;
            dest += 2;
        } else {
